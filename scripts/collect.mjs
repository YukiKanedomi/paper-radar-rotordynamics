...
//
// 使い方:  node scripts/collect.mjs [--topic foil|rotor] [--out <path>]
...
async function fetchArxiv(keywords, max = 14) {
  ...
  // 軸受・ロータ系の arXiv 論文は応用物理/流体/制御系に散在するため複数カテゴリOR
  const CAT = "+AND+%28cat:physics.app-ph+OR+cat:physics.flu-dyn+OR+cat:eess.SY%29";

  // 段階1：完全フレーズOR＋カテゴリ（精度重視）
  let out = await arxivQuery(`%28${phrases.join("+OR+")}%29${CAT}`, max).catch(() => []);
  // 段階2：語OR＋カテゴリ（少なければ緩める）
  if (out.length < ARXIV_MIN) {
    const more = await arxivQuery(`%28${words.join("+OR+")}%29${CAT}`, max).catch(() => []);
    out = dedupeById([...out, ...more]);
  }
  // 段階3：語OR・カテゴリ縛りなし（未クロスリストの軸受・ロータ論文も拾う）
  if (out.length < ARXIV_MIN) {
